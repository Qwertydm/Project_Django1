...
def get_exact_category(request, pk):
    ...
    return render(request, 'exact_category_html', context)
...
def user_cart(request):
    ...
    if request.Cart.objects.filter(user_id=request.user.id):
        main_text = 'Новый заказ!\n\n'
        for i in cart:
            main_text += f'Товар: {i.user_product}\n' \
                         f'Количество: {i.user_product_count}\n'
        bot.send_message(5072564063, main_text)
        cart.delete()
        return redirect('/')
    context = {'cart': cart}
...
def del_from_cart(request, pk):
    product_to_delete = models.Product.object.get(id=pk)
